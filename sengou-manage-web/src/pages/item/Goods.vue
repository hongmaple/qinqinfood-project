    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="offGoods(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="onGoods(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      async onGoods(id){
        ...
        this.$message.confirm("确认要上架该商品吗？")
          .then(() => {
            this.$http.put("/item/goods/on/"+id)
              .then(() => {
                // 重新加载数据
                this.getDataFromServer();
      ...
      async offGoods(id){
        ...
        this.$message.confirm("确认要下架该商品吗？")
          .then(() => {
            this.$http.put("/item/goods/off/"+id)
              .then(() => {
                // 重新加载数据
                this.getDataFromServer();
